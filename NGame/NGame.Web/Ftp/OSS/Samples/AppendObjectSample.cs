        ...
        public static void SyncAppendObject(string bucketName)
        {
            ...
            try
            {
                ...
                using (var fs = File.Open(fileToUpload, FileMode.Open))
                {
                    ...
                    UnityEngine.Debug.LogFormat("Append object succeeded too, next append position:{0}", position);
                }
            }
            ...
            try
            {
                using (var fs = File.Open(fileToUpload, FileMode.Open))
                {
                    do
                    {
                        ...
                        position = result.NextAppendPosition;
                        initCrc = result.HashCrc64Ecma;
                        UnityEngine.Debug.LogFormat("Append object succeeded, next append position:{0}, crc64:{1}", position, initCrc);
