...
namespace NClient.OSS.Samples
{
    public class MultipartUploadSample
    {
        ...
        public static void UploadMultipart(String bucketName)
        {
            ...
            UnityEngine.Debug.LogFormat("Multipart put object:{0} succeeded", key);
        }
        ...
        public static void UploadMultipartCopy(String targetBucket, String targetObject, String sourceBucket, String sourceObject)
        {
            ...
            UnityEngine.Debug.LogFormat(@"Upload multipart copy result : ");
            UnityEngine.Debug.LogFormat(completeResult.Location);
        }
        ...
        public static void ListMultipartUploads(String bucketName)
        {
            ...
            UnityEngine.Debug.LogFormat("Bucket name:" + result.BucketName);
            UnityEngine.Debug.LogFormat("Key marker:" + result.KeyMarker);
            UnityEngine.Debug.LogFormat("Delimiter:" + result.Delimiter);
            UnityEngine.Debug.LogFormat("Prefix:" + result.Prefix);
            UnityEngine.Debug.LogFormat("UploadIdMarker:" + result.UploadIdMarker);

            foreach (var part in result.MultipartUploads)
            {
                UnityEngine.Debug.LogFormat(part.ToString());
            }
        }
                                                  ...
                                                  String uploadId, int partSize)
        {
            ...
            using (var fs = File.Open(fileToUpload, FileMode.Open))
            {
                for (var i = 0; i < partCount; i++)
                {
					...
					UnityEngine.Debug.LogFormat ("finish {0}/{1}", partETags.Count, partCount);
                }
            }
            return partETags;
        }
        ...
        private static void UploadPartCallback(IAsyncResult ar)
        {
            ...
            lock (ctx.SyncLock)
            {
				...
				UnityEngine.Debug.LogFormat ("finish {0}/{1}", ctx.CompletedParts, ctx.TotalParts);
                if (ctx.CompletedParts == ctx.TotalParts)
                {
                    ...
                    UnityEngine.Debug.LogFormat(@"Async upload multipart result : " + completeMultipartUploadResult.Location);
        ...
        private static void UploadPartCopyCallback(IAsyncResult ar)
        {
            ...
            lock (ctx.SyncLock)
            {
                ...
                if (ctx.CompletedParts == ctx.TotalParts)
                {
                    ...
                    var completeMultipartUploadResult = client.CompleteMultipartUpload(completeMultipartUploadRequest);
                    UnityEngine.Debug.LogFormat(@"Async upload multipart copy result : " + completeMultipartUploadResult.Location);
